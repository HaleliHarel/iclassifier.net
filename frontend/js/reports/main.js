	...
	view: vnode => m('div', {
		...
		m('div', {
			...
			Object.entries(witnessData)
				.sort((a, b) => {
					if (a[1].name.trim().toLowerCase() > b[1].name.trim().toLowerCase()) {
						return 1
					} else if (a[1].name.trim().toLowerCase() < b[1].name.trim().toLowerCase()) {
						return -1
					} else {
						return 0
					}
...
let scriptSelectComponent = {
	view: vnode => m('div', {
		...
		m('div', {
			...
			tuplesOrNothing(projectType, 'scripts')
				.map(scriptTuple => m('div', {
					style: {
						display: 'inline-block',
						'background-color': 'palegoldenrod',
						padding: '3px',
...
let posSelectComponent = {
	view: vnode => m('div', {
		...
		m('span', 'Restrict examples to tokens with the following parts of speech: '),
		m('input[type=button]', {
			value: 'Clear selection',
			onclick: () => {
				selectedPOS.clear();
		...
		m('div', {
			...
			extractUniquePOS(tokenData)
				.map(partOfSpeechName => m('div', {
					style: {
						display: 'inline-block',
						'background-color': 'palegoldenrod',
						padding: '3px',
...
function extractUniquePOS(dataDict) {
	...
	for (const key in dataDict) {
		if (!dataDict.hasOwnProperty(key))
			continue;
		if (dataDict[key].pos === null || dataDict[key].pos.trim() === '')
			continue;
