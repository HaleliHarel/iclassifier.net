type ThesaurusEntry = [string, string];

const chineseScripts: ThesaurusEntry[] = [
...
const chineseGenres: ThesaurusEntry[] = [
...
const egyptianGenres: ThesaurusEntry[] = [
...
const egyptianScripts: ThesaurusEntry[] = [
  ["Hieroglyphs", "thot-83"],
...
function buildLookup(entries: ThesaurusEntry[]) {
  return entries.reduce<Record<string, string>>((acc, [label, value]) => {
    acc[value] = label;
    return acc;
  }, {});
}

const thesauriLookup = {
  chinese: {
    scripts: buildLookup(chineseScripts),
    genres: buildLookup(chineseGenres),
  },
  hieroglyphic: {
    scripts: buildLookup(egyptianScripts),
    genres: buildLookup(egyptianGenres),
  },
} as const;

export type ThesaurusCategory = keyof (typeof thesauriLookup)["hieroglyphic"];

const normalizeThesaurusLabel = (label: string) => {
  return label.replace(/^\s*-+\s*/, "");
};

export function getThesaurusLabel(
  projectType: string | undefined,
  category: ThesaurusCategory,
  value: string | null | undefined
) {
  if (!value) return "";
  const typeKey = projectType === "chinese" ? "chinese" : projectType === "hieroglyphic" ? "hieroglyphic" : null;
  if (!typeKey) return value;
  const label = thesauriLookup[typeKey][category][value] || value;
  return normalizeThesaurusLabel(label);
}

export function getThesaurusEntries(
  projectType: string | undefined,
  category: ThesaurusCategory
) {
  const typeKey = projectType === "chinese" ? "chinese" : projectType === "hieroglyphic" ? "hieroglyphic" : null;
  if (!typeKey) return [];
  return thesauriDict[typeKey][category];
}
