...
export default function MapReport() {
  ...
  useEffect(() => {
    if (selectedProjectId) {
      const targetPath = `/project/${selectedProjectId}/map-report`;
  ...
  const extractClassifiers = (mdc_w_markup: string): string[] => {
  ...
  const networkData = useMemo(() => {
    ...
    // Add lemma nodes (light blue color) - limit to top 30 most frequent
    ...
    for (const [lemmaIdStr, count] of sortedLemmas) {
      ...
      if (lemma) {
        ...
        nodes.push({
          id,
          label,
          color: { background: 'lightblue', border: '#4a90e2' },
  ...
  return (
    <SidebarLayout>
      <div className="max-w-7xl mx-auto" id="map-report-content">
        <div className="mb-8">
                ...
                {filteredWitnesses.map((stat) => (
                  <button
                    ...
                    className={cn(
                      ...
                      selectedWitness === stat.witness.id && "bg-teal-50"
                    )}
                  >
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-black text-white">
                  ...
                  <th className="px-4 py-3 text-center">Lemmas</th>
                  <th className="px-4 py-3 text-center">Classifiers</th>
                </tr>
              </thead>
              <tbody>
                {filteredWitnesses.map((stat, idx) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">{stat.witness.script}</td>
                    <td className="px-4 py-3 text-center">{stat.tokenCount}</td>
                    <td className="px-4 py-3 text-center">{stat.lemmaCount}</td>
