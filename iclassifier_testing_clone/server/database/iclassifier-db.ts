...
/**
 * Per-project database cache with reference counting
 */
interface CachedDB {
  instance: iClassifierDB;
  refCount: number;
  lastUsed: number;
}
...
/**
 * Cleanup stale database instances from cache
 */
function cleanupStaleDBs(): void {
  const now = Date.now();
  const entriesToDelete: string[] = [];

  dbCache.forEach((cached, projectId) => {
    if (cached.refCount === 0 && now - cached.lastUsed > CACHE_TTL_MS) {
      cached.instance.close();
      entriesToDelete.push(projectId);
    }
  });

  entriesToDelete.forEach((projectId) => dbCache.delete(projectId));
}

export function resolveDatabaseDir(): string {
  const candidates = [
    process.env.ICLASSIFIER_DATA_PATH,
  ...
  /**
   * Check if candidate is a valid directory
   */
  const isValidDirectory = (candidate: string): boolean => {
    try {
  ...
  const hasNonEmptyDb = (candidate: string) => {
    if (!isValidDirectory(candidate)) return false;
    try {
  ...
  return candidates[0] || path.join(process.cwd(), "data", "projects");
}
...
export function resolveDatabasePath(projectId: string): string {
  ...
  if (!validateProjectId(projectId)) {
    throw new Error(
      `Invalid projectId: "${projectId}". Only alphanumeric characters, dashes, and underscores are allowed.`
    );
  }
  ...
  for (const basePath of candidates) {
    ...
    // Ensure resolved path is within basePath (prevent directory traversal)
    const resolvedDbPath = path.resolve(dbPath);
    const resolvedBasePath = path.resolve(basePath);

    if (!resolvedDbPath.startsWith(resolvedBasePath)) {
      console.warn(
        `[DB] Attempted path traversal detected: ${projectId} resolves outside base path`
      );
      continue;
    }
...
export class iClassifierDB {
  ...
  private extractClassifiersFromMarkup(markup: string | null | undefined): string[] {
    if (!markup) return [];
    let inside = false;
    let buffer = "";
    const results: string[] = [];
    for (let i = 0; i < markup.length; i += 1) {
      const char = markup.charAt(i);
      if (char === "~") {
        if (!inside) {
          inside = true;
          buffer = "";
  ...
  private async getClassifierValuesFromTokens(
    search: string,
    limit: number,
    offset: number
    ...
    tokens.forEach((token: any) => {
      const clfs = this.extractClassifiersFromMarkup(token.mdc_w_markup);
      clfs.forEach((clf) => classifierSet.add(clf));
    });

    let items = Array.from(classifierSet);
    if (search) {
      const query = search.toLowerCase();
      items = items.filter((classifier) => classifier.toLowerCase().includes(query));
    }
    items.sort((a, b) => a.localeCompare(b));

    const total = items.length;
    const safeOffset = Math.max(0, offset);
    const safeLimit = Number.isFinite(limit) && limit >= 0 ? limit : total;
  ...
  async getClassifierValuesPaged(
    ...
    try {
      ...
      if (!config.table || !config.classifierColumn) {
        return await this.getClassifierValuesFromTokens(search, limit, offset);
      }
      ...
      if (total === 0) {
        const fallback = await this.getClassifierValuesFromTokens(search, limit, offset);
        if (fallback.total > 0 || items.length === 0) {
          return fallback;
        }
      }
...
/**
 * Get database instance for a project with caching
 * Returns a cached instance if available, otherwise creates a new one
 *
 * IMPORTANT: Call releaseProjectDB() when done with the database
 */
export function getProjectDB(projectId: string): iClassifierDB {
  ...
  // Create new instance
  const instance = new iClassifierDB(projectId);
  dbCache.set(projectId, {
    instance,
    refCount: 1,
    lastUsed: Date.now(),
  });
...
/**
 * Release a database instance (decrement reference count)
 * Call this after you're done using the database
 */
export function releaseProjectDB(projectId: string): void {
  if (!dbCache.has(projectId)) {
    return;
  }
