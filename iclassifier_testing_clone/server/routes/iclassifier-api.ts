...
import { getProjectDB, releaseProjectDB, resolveDatabaseDir } from "../database/iclassifier-db";
import fs from "fs";
...
router.get("/:projectId/lemmas", async (req: Request, res: Response) => {
  ...
  try {
    ...
    (lemmas as any[]).forEach((lemma) => {
      const lexicalField = lemma.lexical_field || (lemma as any).semantic || "";
      lemmaMap[lemma.id] = {
        id: lemma.id,
    ...
    const statusCode = error instanceof Error && error.message.includes("Invalid projectId") ? 400 : 500;
    res.status(statusCode).json({ error: "Failed to fetch lemmas" });
  } finally {
    if (db) releaseProjectDB(projectId);
  }
});
...
router.get("/:projectId/lemmas/paged", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const lemmas = items.map((lemma) => ({
      ...
      transliteration: lemma.transliteration || "",
      meaning: lemma.meaning || "",
      lexical_field: lemma.lexical_field || (lemma as any).semantic || "",
      concept: lemma.concept || "",
...
router.get("/:projectId/tokens", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const statusCode = error instanceof Error && error.message.includes("Invalid projectId") ? 400 : 500;
    res.status(statusCode).json({ error: "Failed to fetch tokens" });
  } finally {
    if (db) releaseProjectDB(projectId);
  }
});
...
router.get("/:projectId/witnesses", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const statusCode = error instanceof Error && error.message.includes("Invalid projectId") ? 400 : 500;
    res.status(statusCode).json({ error: "Failed to fetch witnesses" });
  } finally {
    if (db) releaseProjectDB(projectId);
  }
});
...
router.get("/:projectId/scripts", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const statusCode = error instanceof Error && error.message.includes("Invalid projectId") ? 400 : 500;
    res.status(statusCode).json({ error: "Failed to fetch scripts" });
  } finally {
    if (db) releaseProjectDB(projectId);
  }
});
...
router.get("/:projectId/classifiers", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const statusCode = error instanceof Error && error.message.includes("Invalid projectId") ? 400 : 500;
    res.status(statusCode).json({ error: "Failed to fetch classifiers" });
  } finally {
    if (db) releaseProjectDB(projectId);
  }
});
...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const statusCode = error instanceof Error && error.message.includes("Invalid projectId") ? 400 : 500;
      res.status(statusCode).json({ error: "Failed to fetch classifier metadata" });
    } finally {
      if (db) releaseProjectDB(projectId);
    }
  }
...
router.get("/:projectId/classifier-meanings", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const statusCode = error instanceof Error && error.message.includes("Invalid projectId") ? 400 : 500;
    res.status(statusCode).json({ error: "Failed to fetch classifier meanings" });
  } finally {
    if (db) releaseProjectDB(projectId);
  }
});
...
router.get("/:projectId/query", async (req: Request, res: Response) => {
  ...
  const posFilter = String(req.query.pos || "")
    .split(",")
    .map((value) => value.trim())
    .filter(Boolean);
  const commentFilter = String(req.query.comment || "").trim();
  ...
  try {
    ...
    if (posFilter.length > 0) {
      const posSet = new Set(posFilter.map((value) => value.toLowerCase()));
      tokens = tokens.filter((token) => {
        const tokenPos = String(token.pos || "").trim().toLowerCase();
        return tokenPos && posSet.has(tokenPos);
      });
    }

    if (commentFilter) {
      const needle = commentFilter.toLowerCase();
      tokens = tokens.filter((token) => {
        const comments = String(token.comments || "").toLowerCase();
        const signComments = String(token.sign_comments || "").toLowerCase();
        return comments.includes(needle) || signComments.includes(needle);
      });
    }
    ...
    const statusCode = error instanceof Error && error.message.includes("Invalid projectId") ? 400 : 500;
    res.status(statusCode).json({ error: "Failed to run query" });
  } finally {
    if (db) releaseProjectDB(projectId);
  }
});
...
router.get("/:projectId/full", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const statusCode = error instanceof Error && error.message.includes("Invalid projectId") ? 400 : 500;
    res.status(statusCode).json({ error: "Failed to fetch project data" });
  } finally {
    if (db) releaseProjectDB(projectId);
  }
});
